...
exports.forgotPassword = async (inputEmail) => {
  ...
  try {
    ...
    const userDetails = await User.findOne({ email: inputEmail }).session(
      session
    );
    ...
    if (userDetails && Object.keys(userDetails).length > 0) {
      const createRequest = new forgotPassword({
        id: uuid_data,
    ...
    console.log(error);
    return {
      message: "Reset Email Unsuccessful, No User exists with this email Id",
      success: false,
    };
  }
};
  ...
  try {
    ...
    if (userData && Object.keys(userData).length > 0) {
      if (userData.isactive) {
        ...
        const t = await forgotPassword
          .findOneAndUpdate({ id: uuid }, { $set: { isactive: false } })
          .session(session);
...
exports.updateNewPassword = async (reqpasswordid, newpassword) => {
  ...
  try {
    const forgotpasswordData = await forgotPassword
      .findOne({ id: reqpasswordid })
      .session(session);
    //console.log(forgotpasswordData);

    if (forgotpasswordData) {
      const userData = await User.findOne({
        _id: forgotpasswordData.userId,
      }).session(session);
      //console.log(userData);
      ...
      if (userData) {
        ...
        session.endSession();
        return {
          message: "Successfully update the new password",
          success: true,
        };
      } else {
        await session.abortTransaction();
